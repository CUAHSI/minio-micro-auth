...
def test_public_to_private_view(action_json):
    # populate the cache
    ...
    with open("tests/json_payloads/access_control_changed/from_public/to_private.json", "r") as file:
    ...
    check_private_view_authorization(request_body, "a2c0df5bf3eb4d8c8a34beaffe169f91")
...
def test_public_to_private_edit(action_json):
    # populate the cache
    ...
    check_private_edit_authorization(request_body, "a2c0df5bf3eb4d8c8a34beaffe169f91")
...
@pytest.mark.parametrize("action_json", [
    ...
    "user1_list_bucket.json",
    "user1_list_objects_v2.json"])    
def test_public_to_private_link_view(action_json):
    # populate the cache
    with open(f"tests/json_payloads/view_authorization/{action_json}", "r") as file:
        request_body = file.read()
    check_public_view_authorization(request_body)
    ...
    with open("tests/json_payloads/access_control_changed/from_public/to_private_sharing.json", "r") as file:
        access_control_changed_payload = file.read()
    response = client.post("/hook/", data=access_control_changed_payload)
    assert response.status_code == 204
    
    # check private link view authorization
    check_private_link_view_authorization(request_body, "a2c0df5bf3eb4d8c8a34beaffe169f91")
...
@pytest.mark.parametrize("action_json", [
    ...
    "user1_delete_object.json",
    "user1_delete_objects.json"])
def test_public_to_private_link_edit(action_json):
    # populate the cache
    with open(f"tests/json_payloads/edit_authorization/{action_json}", "r") as file:
        request_body = file.read()
    check_public_edit_authorization(request_body)
    ...
    # check private link view authorization
    check_private_link_edit_authorization(request_body, "a2c0df5bf3eb4d8c8a34beaffe169f91")
